                    ...
                    <div class="row">
                      <div class="col-4 text-left">
                        <dropdown1
                          ...
                          "
                          @my-approved="
                            handleApprove('myApproved', userApprovedCancel)
                          "
                          @my-rejected="
                            handleApprove('myRejected', userRejectedCancel)
                          "
                      ...
                      <div class="col-4 text-center">
                        Pending Canceled Leave Details
                      </div>
                ...
                <q-card class="roundedd1">
                  <template v-if="$q.screen.width > 900">
                    ...
                    <template v-else>
                      <virtualScroll
                        :items="filteredLeaveRows"
                        :columns="userApprovedRejectedcolumns"
                        :LeaveTypeLabels="LeaveTypeLabels"
                        :cancelView="true"
                      />
                      <div
                ...
                <template v-if="$q.screen.width > 900">
                      ...
                      <div class="col-4 text-left">
                        <dropdown2
                          ...
                          @show-rejected-leaves="
                            showApprovedCancelLeaves = false
                          "
                          :cancelLeave="true"
                        />
                      </div>
                  ...
                  <template v-else>
                    <virtualScrollResult
                      ...
                      :columns="approvedRejectedCancelcolumns"
                      :LeaveTypeLabels="LeaveTypeLabels"
                      :cancelView="true"
                    />
                    <div
                      v-if="sortedPendingCancelLeave.length === 0"
                      class="text-center"
                    >
                      <q-icon name="warning" style="font-size: 30px" />
                      No data available
  ...
  computed: {
    ...
    filteredCancelLeaveRows() {
      if (!this.selectedDeptDescription) {
        ...
        this.filterDialog = false;
        return this.sortedPendingCancelLeave.filter(
          (row) => row.deptDescription === this.selectedDeptDescription,
        );
      }
    },
  ...
  methods: {
    ...
    async getPendingCancel() {
      try {
        ...
        const localStorageShowLevel1Card =
          localStorage.getItem("showLevel1Card");
        const localStorageShowMyCancelApproved = localStorage.getItem(
          "showMyCancelApproved",
        );
        const localStorageShowMyCancelRejected = localStorage.getItem(
          "showMyCancelRejected",
        );

        if (
          localStorageShowLevel1Card !== null &&
          ...
          this.showMyCancelApproved =
            localStorageShowMyCancelApproved === "true";
          this.showMyCancelRejected =
            localStorageShowMyCancelRejected === "true";
        } else {
        ...
        deptDescriptionsOptions = [
          ...new Set(pendingLeave.map((leave) => leave.deptDescription)),
        ];
    ...
    handleApprove(cardType, dataList) {
      ...
      deptDescriptionsOptions = [
        ...new Set(dataList.map((leave) => leave.deptDescription)),
      ];
    },
  ...
  created() {
    ...
    window.addEventListener("beforeunload", () => {
      ...
      localStorage.setItem(
        "showMyCancelApproved",
        this.showMyCancelApproved.toString(),
      );
      localStorage.setItem(
        "showMyCancelRejected",
        this.showMyCancelRejected.toString(),
      );
    });
  },
};
